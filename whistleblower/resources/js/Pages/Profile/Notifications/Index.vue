...
const searchField = ["topic", "classification", "created_at", "read"];
const sortBy= "";
const sortType = "";

const headers = [
    { text: "Subject", value: "topic", sortable: true},
    { text: "From", value: "classification", sortable: true},
                ...
                >
                    <div class="rounded-t bg-white mb-0 px-6 py-6">
                        <div class="text-center flex justify-between">
                            <h6 class="text-blueGray-700 text-xl font-bold mr-2">{{$t('Notifications')}}</h6>
                    ...
                    <div class="block w-full overflow-x-auto">
                        <Vue3EasyDataTable
                            ...
                            header-item-class-name="bg-blueGray-50 text-blueGray-500 border-blueGray-100 x-6 align-middle border border-solid py-3 text-xs uppercase border-l-0 border-r-0 font-semibold text-left px-6"
                            body-row-class-name=""
                            body-expand-row-class-name=""
                            body-item-class-name="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs p-4 text-left"
                        ...
                        >
                            ...
                            <template #item-topic="value">
                              <Decryption :occurrences="value.occurrence" :encryptedText="value.data.topic" />
                            </template>
                            <template #item-classification="{data}">
                              {{data.classification}}
                            </template>
                            <template #item-created_at="{created_at}">
                              <DateFormat :date="created_at" hasTime="true" />
                            </template>
                            <template #item-read="{read_at}">
                              <span v-if="read_at">
                                <DateFormat :date="read_at" hasTime="true" />
                              </span>
                              <span v-else class="text-red-500">
                                {{$t('Unread')}}
                              </span>
                            </template>
                            <template #item-actions="notification">
                              <Link
                                class="bg-emerald-400 text-white active:bg-emerald-600 text-xs font-bold uppercase px-3 py-1 rounded outline-none focus:outline-none mr-1 mb-1 ease-linear transition-all duration-150" title="Back"
