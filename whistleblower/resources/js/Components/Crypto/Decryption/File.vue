...
<template>
  <a href="#" class="flex items-center" @click="openFileToDownload(props.occurrences,props.name, props.encryptedText)" v-if="props.type === 1">
    ...
    <div class="w-6 flex items-center justify-center" title="Download">
  ...
  <a v-else href="#" @click="openFileToDownload(props.occurrences,props.name, props.encryptedText)">{{props.name}}</a>
</template>
...
function openFileToDownload(occurrence,name ,base64Data) {
  let data = decryptByAESkey(occurrence, base64Data);
